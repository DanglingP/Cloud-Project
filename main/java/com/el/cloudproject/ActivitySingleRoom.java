...
/**
 * Created by Omar Sheikh on 4/27/2018.
 */
public class ActivitySingleRoom extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        CreateUserBar(ActivitySingleRoom.this, "Room");
        LoadDataFromIntent();
        InitializeVariables();
        SetListners();
    ...
    public void InitializeVariables(){
        ...
        bookCheckOutButton = (Button) findViewById(R.id.singleRoomBookCheckOutButton);
        if(currentRoom.getAvailability() == 0)
            bookCheckOutButton.setText("Checkout");
        else
            bookCheckOutButton.setText("Book");

    }
    public void SetListners(){
        bookCheckOutButton.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                if(currentRoom.getAvailability() == 0) {
                    new AlertDialog.Builder(ActivitySingleRoom.this)
                            .setTitle("Checkout")
                            .setMessage("Do you wish to checkout")
                            .setIcon(android.R.drawable.ic_dialog_alert)
                            .setPositiveButton(android.R.string.yes, new DialogInterface.OnClickListener() {
                                public void onClick(DialogInterface dialog, int whichButton) {
                                    rootURL.child(FirebaseConstants.bookedClassURL).child(currentUser.getUsername()).child(currentRoom.getRoomNumber()).removeValue();
                                    rootURL.child(FirebaseConstants.roomClassURL).child(currentRoom.getRoomNumber()).child(FirebaseConstants.roomAvailability).setValue(1);
                                    Intent intent = new Intent(ActivitySingleRoom.this, ActivityAddReview.class);
                    ...
                    rootURL.child(FirebaseConstants.bookedClassURL).child(currentUser.getUsername()).child(currentRoom.getRoomNumber()).setValue("1");
                    rootURL.child(FirebaseConstants.roomClassURL).child(currentRoom.getRoomNumber()).child(FirebaseConstants.roomAvailability).setValue(0);
                    bookCheckOutButton.setText("Checkout");
                    currentRoom.setAvailability(0);
                }
            }
        });
    }
